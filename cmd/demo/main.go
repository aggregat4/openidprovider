...
func main() {
	...
	logging.Info(logger, "Loaded configuration from configFile={ConfigFile}", *configFile)

	// Get OpenID Provider configuration
	providerConfig, err := getOpenIDProviderConfig(config.OpenIDProvider)
	if err != nil {
		logging.Fatal(logger, "Error getting OpenID Provider configuration: {Error}", err)
	}
	logging.Info(logger, "Successfully retrieved OpenID Provider configuration from provider={Provider}", config.OpenIDProvider)
	...
	if err := http.ListenAndServe(":8080", nil); err != nil {
		logging.Fatal(logger, "HTTP server failed: {Error}", err)
	}
}
...
func getOpenIDProviderConfig(providerURL string) (*OpenIDProviderConfig, error) {
	...
	logging.Info(logger, "Fetching OpenID Provider configuration from discoveryURL={DiscoveryURL}", discoveryURL)
...
func readConfig(path string) Config {
	...
	if err != nil {
		logging.Fatal(logger, "Error reading config file: {Error}", err)
	}

	var config Config
	if err := json.Unmarshal(file, &config); err != nil {
		logging.Fatal(logger, "Error parsing config file: {Error}", err)
	}
...
func handlePublic(w http.ResponseWriter, r *http.Request) {
	logging.Info(logger, "Handling public request method={Method} path={Path}", r.Method, r.URL.Path)
	publicTemplate.Execute(w, nil)
}

func handleProtected(w http.ResponseWriter, r *http.Request) {
	logging.Info(logger, "Handling protected request method={Method} path={Path}", r.Method, r.URL.Path)
	...
	logging.Info(logger, "Rendering protected page for user email={Email}", data.Email)
...
func handleLogin(w http.ResponseWriter, r *http.Request, config Config, providerConfig *OpenIDProviderConfig) {
	logging.Info(logger, "Handling login request method={Method} path={Path}", r.Method, r.URL.Path)
	...
	logging.Info(logger, "Redirecting to authorization endpoint authURL={AuthURL}", authURL)
...
func handleCallback(w http.ResponseWriter, r *http.Request, config Config, providerConfig *OpenIDProviderConfig) {
	logging.Info(logger, "Handling callback request method={Method} path={Path}", r.Method, r.URL.Path)
	...
	logging.Info(logger, "Making token request to tokenEndpoint={TokenEndpoint}", providerConfig.TokenEndpoint)
...
func handleLogout(w http.ResponseWriter, r *http.Request) {
	logging.Info(logger, "Handling logout request method={Method} path={Path}", r.Method, r.URL.Path)
...
func parseToken(token string) (map[string]interface{}, error) {
	...
	if len(parts) != 3 {
		logging.Error(logger, "Invalid token format: expected 3 parts, got {PartCount}", len(parts))
...
func handleEmails(w http.ResponseWriter, r *http.Request) {
	logging.Info(logger, "Handling emails request method={Method} path={Path}", r.Method, r.URL.Path)
